...
function ExplorerPage() {
  ...
  const [mode, setMode] = useState<"recommendation" | "catalog">("recommendation");
  ...
  const effectiveMode = hasRecommendations ? mode : "catalog";
  const selectedRecId = recommendationId || data.recommendations[0]?.id || "";
  const selectedRec = data.recommendations.find((item) => item.id === selectedRecId);
  const selectedCatalog = data.catalog.find((c) => c.slug === catalogSlug) ?? data.catalog[0];
  const selectedTitle = effectiveMode === "recommendation" ? selectedRec?.career_title : selectedCatalog?.title;
  const canStart = effectiveMode === "recommendation" ? !!selectedRec : !!selectedCatalog;

  const start = async () => {
    setBusy(true); setError(""); setFeedback(null); setResponse(""); setEnjoyment(0);
    try {
      const payload = effectiveMode === "recommendation"
        ? { source: "recommendation" as const, recommendationId: selectedRecId }
        : { source: "catalog" as const, careerSlug: selectedCatalog!.slug };
  ...
  const finish = async () => {
    ...
    try {
      const payload = effectiveMode === "recommendation"
        ? { source: "recommendation" as const, recommendationId: selectedRecId, task, response, enjoyment }
        : { source: "catalog" as const, careerSlug: selectedCatalog!.slug, task, response, enjoyment };
  ...
  const groupedCatalog = data.catalog.reduce<Record<string, typeof data.catalog>>((acc, item) => {
